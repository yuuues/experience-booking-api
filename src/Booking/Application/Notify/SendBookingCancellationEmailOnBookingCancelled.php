...
#[AsMessageHandler]
final readonly class SendBookingCancellationEmailOnBookingCancelled
{
    ...
    public function __invoke(BookingCancelled $event): void
    {
        ...
        if ($this->sent->wasSent($reference, BookingEmail::TYPE_CANCELLATION)) {
            return;
        }
        ...
        $this->mailer->send(new BookingEmail(
            ...
            type: BookingEmail::TYPE_CANCELLATION,
            reference: $reference->value,
            seats: $event->seats,
            totalAmount: 0,
            totalCurrency: $session->price()->currency,
            sessionStartsAt: $session->startsAt()->toAtom(),
        ));

        $this->sent->markSent($reference, BookingEmail::TYPE_CANCELLATION);
    }
}
